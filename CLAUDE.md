...
**Avatar sources.** `User.avatarUrl` holds one of four user-curated shapes, all resolved to the `picture` claim identically: null (a deterministic identicon, `picture` omitted — nothing external to disclose), a Gravatar URL, a free-text https link, or an UPLOADED photo. An upload is stored as a bytea blob in its own `UserAvatar` table (`{ userId @id, data Bytes, contentType, updatedAt }`, cascade-deleted with the user, kept off the hot User-row reads); `avatarUrl` is set to the absolute internal serve route `/api/users/<id>/avatar?v=<updatedAt ms>` (origin from `AUTH_URL`, never a client Host header). `uploadAvatarAction` is the only writer: it validates STRICTLY server-side (`src/lib/avatar-image.ts`, a plain module — a "use server" file may export only async fns) — 512 KB cap, MAGIC-BYTE sniff accepting only PNG/JPEG/WebP with the stored `contentType` being the sniffed type (SVG and MIME-spoofed uploads rejected, since an inline SVG is stored-XSS). The public serve route (`src/app/api/users/[userId]/avatar/route.ts`) returns the bytes with `X-Content-Type-Options: nosniff`, `Content-Disposition: inline`, a version-keyed `Cache-Control`, and 404 when absent. Switching back to any non-upload avatar deletes the blob. Not stripped: EXIF metadata (bytes are stored verbatim, no re-encode/no `sharp`).
