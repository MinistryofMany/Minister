...
const ORIGINAL = {
  ...
  CAPTURE: process.env.TESSERA_MAIL_CAPTURE_FILE,
};
...
describe("sendMail", () => {
  ...
  beforeEach(() => {
    ...
    setEnv("TESSERA_MAIL_CAPTURE_FILE", undefined);
  ...
  afterEach(() => {
    ...
    setEnv("TESSERA_MAIL_CAPTURE_FILE", ORIGINAL.CAPTURE);
    vi.unstubAllGlobals();
  });
  ...
  describe("Resend transport", () => {
    beforeEach(() => {
      ...
      setEnv("MAIL_FROM", "Tessera <noreply@example.com>");
    });
    ...
    it("POSTs the message to the Resend API with auth + correct shape", async () => {
      ...
      expect(body).toEqual({
        from: "Tessera <noreply@example.com>",
        to: ["bob@example.com"],
        subject: "Verify",
        text: "click here",
        html: "<p>click here</p>",
