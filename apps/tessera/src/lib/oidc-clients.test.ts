...
describe("isRegisteredRedirectUri", () => {
  it("requires exact match (RFC 6749 §3.1.2.2)", () => {
    const client = {
      redirectUris: ["http://localhost:3100/api/auth/callback/tessera"],
    };
    expect(
      isRegisteredRedirectUri(
        client,
        "http://localhost:3100/api/auth/callback/tessera",
      ),
    ).toBe(true);
  });
  ...
  it("rejects a path-suffix-extension attack", () => {
    ...
    expect(
      isRegisteredRedirectUri(
        client,
        "http://localhost:3100/api/auth/callback/tessera/extra",
      ),
    ).toBe(false);
  });
