...
export const emailDomainPlugin: Plugin = {
  manifest: {
    ...
    name: "Email domain",
    description:
      "Prove you control an email address at a given domain. Verified via a one-time link sent to that address — Tessera never stores the email itself, only the domain.",
  ...
  async handleStep(state, input, ctx) {
    switch (state.currentStep.id) {
      case STEP_FORM: {
        ...
        await ctx.sendMail({
          to: email,
          subject: "Verify your email for Tessera",
          text: [
            "Someone (hopefully you) asked Tessera to issue a badge proving",
  ...
  return {
    ...
    currentStep: {
      ...
      payload: {
        ...
        description:
          "Enter any email address at the domain you want to attest. Tessera will email a verification link; clicking it issues the badge.",
        fields: [
          {
            name: "email",
            label: "Email address",
