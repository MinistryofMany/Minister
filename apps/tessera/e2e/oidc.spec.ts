...
test("authorization-code + PKCE dance end-to-end", async ({
  ...
  const badges = idClaims.tessera_badges as string[];
  expect(Array.isArray(badges)).toBe(true);
  expect(badges.length).toBeGreaterThanOrEqual(1);
  for (const vc of badges) {
    const vcClaims = decodeJwtPayload(vc);
    expect(String(vcClaims.sub)).toContain("did:web:tessera.local:users:");
  }

  // The access token carries no raw user id — userinfo resolves it.
  const accessClaims = decodeJwtPayload(tokens.access_token);
  expect(accessClaims.tessera_uid).toBeUndefined();
