...
test("'sign out of all devices' revokes the current session", async ({
  browser,
}) => {
  ...
  await page
    .getByRole("button", { name: /Sign out of all devices/i })
    .click();
...
test("an account-gated share link blocks anonymous viewers", async ({
  browser,
}) => {
  ...
  await ownerPage.getByText("Require a Tessera account").click(); // the gate
  ...
  await expect(
    anonPage.getByRole("heading", { name: "Sign-in required" }),
  ).toBeVisible();
