import NextAuth from "next-auth";

// Stage 4 — demo client signs in via Minister using a generic Auth.js
// OIDC provider. Minister publishes /.well-known/openid-configuration,
// so we hand Auth.js the issuer URL and it discovers the rest.
//
// We use JWT-strategy sessions on the RP side too (no DB on the demo
// client) and copy the relevant claims from the Minister ID token onto
// our session: pairwise sub, name/picture, minister_badges (the array
// of VC JWTs Minister issued for the badges this user consented to
// disclose).
export const { handlers, auth, signIn, signOut } = NextAuth({
  session: { strategy: "jwt", maxAge: 60 * 60 }, // 1h — shorter than Minister's 24h
  providers: [
    {
      id: "minister",
      name: "Minister",
      type: "oidc",
      issuer: process.env.MINISTER_ISSUER_URL ?? "http://localhost:3000",
      clientId: process.env.MINISTER_CLIENT_ID ?? "",
      clientSecret: process.env.MINISTER_CLIENT_SECRET ?? "",
      // Scopes we want to request. The user picks per-badge inside the
      // Minister consent screen; we ask broadly here and accept whatever
      // they approve.
      authorization: {
        params: {
          scope: "openid profile badge:email-domain badge:invite-code",
        },
      },
      // The discovery doc says subject_types: ["pairwise"] but Auth.js
      // doesn't care — it just verifies the id_token signature via
      // /jwks.json and reads the claims.
      checks: ["pkce", "state", "nonce"],
    },
  ],
  callbacks: {
    // The OIDC provider hands us id_token; we squirrel away pieces we
    // care about so server components don't have to keep re-decoding.
    async jwt({ token, account, profile }) {
      if (account?.id_token) {
        token.idToken = account.id_token;
        token.accessToken = account.access_token;
      }
      if (profile) {
        const p = profile as {
          sub?: unknown;
          name?: unknown;
          picture?: unknown;
          minister_badges?: unknown;
        };
        if (typeof p.sub === "string") token.ministerSub = p.sub;
        if (typeof p.name === "string") token.ministerName = p.name;
        if (typeof p.picture === "string") token.ministerPicture = p.picture;
        if (Array.isArray(p.minister_badges)) {
          token.ministerBadges = p.minister_badges.filter(
            (x): x is string => typeof x === "string",
          );
        }
      }
      return token;
    },
    async session({ session, token }) {
      session.ministerSub = (token.ministerSub as string | undefined) ?? null;
      session.ministerName = (token.ministerName as string | undefined) ?? null;
      session.ministerPicture = (token.ministerPicture as string | undefined) ?? null;
      session.ministerBadges = (token.ministerBadges as string[] | undefined) ?? [];
      session.idToken = (token.idToken as string | undefined) ?? null;
      session.accessToken = (token.accessToken as string | undefined) ?? null;
      return session;
    },
  },
});

declare module "next-auth" {
  interface Session {
    ministerSub: string | null;
    ministerName: string | null;
    ministerPicture: string | null;
    ministerBadges: string[];
    idToken: string | null;
    accessToken: string | null;
  }
}
