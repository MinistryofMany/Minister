...
test("ban locks the user out immediately; unban restores access", async ({
  page,
  browser,
}) => {
  ...
  await expect(
    victimPage.getByRole("heading", { name: "Profile" }),
  ).toBeVisible();
  ...
  await expect(
    victimPage.getByRole("button", { name: "Email me a magic link" }),
  ).toBeVisible();
...
test("promote and demote from the Users tab; self-row has no buttons", async ({
  page,
}) => {
  acceptDialogs(page);
  await page.goto("/admin/users");
...
test("OIDC client lifecycle: register public client, delete it", async ({
  page,
}) => {
  acceptDialogs(page);
  ...
  await page
    .getByPlaceholder(/theirapp\.com/)
    .fill("https://e2e.example.com/callback");
  await page
    .getByRole("checkbox", { name: "Public client", exact: false })
    .check();
