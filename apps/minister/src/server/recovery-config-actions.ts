...
async function broadcastRecoveryConfigChange(subject: string, body: string): Promise<void> {
  // The config change is already committed + audited before this runs, so the
  // WHOLE broadcast — including the admin-list read — is best-effort: a DB blip
  // enumerating admins must NOT surface as a failed change. Per-recipient sends
  // are additionally guarded so one bad address doesn't drop the rest.
  try {
    ...
    console.error(
      "[recovery-config] admin broadcast failed to enumerate/notify admins:",
      err instanceof Error ? err.message : err,
    );
  }
}
...
export async function updateRecoveryWeight(
  ...
  return run(async () => {
    ...
    const { effectiveAt, before } = await prisma.$transaction(
      async (tx) => {
        ...
        // stays valid.
        const soloError = soloBlockError(
          recoveryWeight,
          worstThreshold(cfg),
          row.allowSoloRecovery,
        );
