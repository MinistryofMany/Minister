...
import { ACCESS_TOKEN_TTL, pairwiseJti, pairwiseSub } from "@/lib/oidc-tokens";
import { prisma } from "@/lib/prisma";
import { shareLinkPairwiseJti, shareLinkPairwiseSub } from "@/lib/share-links";
...
// ---------------------------------------------------------------------------
// FROZEN GOLDEN VECTORS — cross-repo pairwise-HMAC fixtures.
//
// These are the byte-equality fixtures Signet's `/prf/pairwise` interop tests
// import in Phase 2 (build plan §2.1): Signet exposes ONE generic keyed-HMAC
// oracle and Minister composes the tagged input, so a fixture is just
// (secret bytes, exact tagged input string, base64url output). All four
// derivation families are pinned here.
//
// The values are FROZEN FOREVER. They were generated by invoking the real
// derivation functions with the fixed secret + fixed inputs below; a change to
// any output means the wire encoding changed, which is a breaking change that
// must re-pin BOTH repos deliberately, never silently. Do NOT edit the outputs
// to make a failing test pass — a mismatch is the signal.
//
// GOLDEN_PAIRWISE_SECRET, the inputs, and the outputs are the exact triples
// Signet asserts; keep them labeled and stable.
const GOLDEN_PAIRWISE_SECRET = "minister-golden-vector-secret-v1-do-not-change!!";

// Fixed inputs. The clientId is `mc_`-prefixed (Phase 0b charset guard) so the
// fixture also exercises a realistic Minister-generated id shape.
const GOLDEN = {
...
// The exact tagged-input strings Minister feeds the HMAC (what Signet's generic
// oracle receives), for reference in the cross-language fixture.
const GOLDEN_INPUTS = {
  pairwiseSub: `${GOLDEN.userId}:${GOLDEN.clientId}`,
  pairwiseJti: `jti:${GOLDEN.badgeId}:${GOLDEN.clientId}`,
  shareLinkPairwiseSub: `sharelink:${GOLDEN.userId}:${GOLDEN.shareLinkId}`,
  shareLinkPairwiseJti: `jti:sharelink:${GOLDEN.badgeId}:${GOLDEN.shareLinkId}`,
} as const;

// The frozen base64url outputs: HMAC-SHA256(GOLDEN_PAIRWISE_SECRET, <input>).
const GOLDEN_OUTPUTS = {
  pairwiseSub: "xOfT05jnZI0r8hweyDLf7GnlAlPoUhHHoUsKH49Olm0",
  pairwiseJti: "5fIc0YcinsYRBEf1J6aZXcoKuxmDStXGch6Rk_bDylM",
  shareLinkPairwiseSub: "3Wfr4iEXijtFDIQ9JkYamk6r427jpcY4ApbNbShi9sY",
  shareLinkPairwiseJti: "8ITdmHQXFlAukLUGdhAOqexFVwIEbnQFKvOnHy3LoOo",
} as const;

describe("pairwise derivation golden vectors (FROZEN — Signet /prf/pairwise fixtures)", () => {
  let SAVED_SECRET: string | undefined;

  beforeAll(() => {
    SAVED_SECRET = process.env.OIDC_PAIRWISE_SECRET;
    process.env.OIDC_PAIRWISE_SECRET = GOLDEN_PAIRWISE_SECRET;
  });
  afterAll(() => {
    if (SAVED_SECRET === undefined) delete process.env.OIDC_PAIRWISE_SECRET;
    else process.env.OIDC_PAIRWISE_SECRET = SAVED_SECRET;
  });

  it("pairwiseSub matches the frozen vector", () => {
    expect(pairwiseSub(GOLDEN.userId, GOLDEN.clientId)).toBe(GOLDEN_OUTPUTS.pairwiseSub);
  });

  it("pairwiseJti matches the frozen vector", () => {
    expect(pairwiseJti(GOLDEN.badgeId, GOLDEN.clientId)).toBe(GOLDEN_OUTPUTS.pairwiseJti);
  });

  it("shareLinkPairwiseSub matches the frozen vector", () => {
    expect(shareLinkPairwiseSub(GOLDEN.userId, GOLDEN.shareLinkId)).toBe(
      GOLDEN_OUTPUTS.shareLinkPairwiseSub,
    );
  });

  it("shareLinkPairwiseJti matches the frozen vector", () => {
    expect(shareLinkPairwiseJti(GOLDEN.badgeId, GOLDEN.shareLinkId)).toBe(
      GOLDEN_OUTPUTS.shareLinkPairwiseJti,
    );
  });

  // Independent re-derivation of the same fixtures directly from the tagged
  // input strings, so the cross-repo fixture (secret, input, output) is pinned
  // even if a derivation function's input composition ever drifted.
  it("every family equals HMAC-SHA256(secret, taggedInput) for the recorded inputs", () => {
    const hmac = (msg: string) =>
      createHmac("sha256", GOLDEN_PAIRWISE_SECRET).update(msg).digest("base64url");
    expect(hmac(GOLDEN_INPUTS.pairwiseSub)).toBe(GOLDEN_OUTPUTS.pairwiseSub);
    expect(hmac(GOLDEN_INPUTS.pairwiseJti)).toBe(GOLDEN_OUTPUTS.pairwiseJti);
    expect(hmac(GOLDEN_INPUTS.shareLinkPairwiseSub)).toBe(GOLDEN_OUTPUTS.shareLinkPairwiseSub);
    expect(hmac(GOLDEN_INPUTS.shareLinkPairwiseJti)).toBe(GOLDEN_OUTPUTS.shareLinkPairwiseJti);
  });
});
