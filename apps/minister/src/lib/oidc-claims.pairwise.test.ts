...
// stamp into that RP's id_token (pairwiseSub, absent a merge override).
async function discloseTo(clientId: string) {
...
describe("loadApprovedBadgeJwts — pairwise disclosure (MIN-1)", () => {
  ...
  it("scopes the DB query to the owning user (a stale id can't surface another user's badge)", async () => {
    ...
    expect(prisma.badge.findMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: { userId: USER, id: { in: [BADGE_ID] } } }),
    );
  });
});
