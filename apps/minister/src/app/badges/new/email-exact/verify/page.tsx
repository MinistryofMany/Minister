...
export default async function EmailExactVerifyPage({ searchParams }: PageProps) {
    ...
    return (
      <ResultShell title="Missing token">
        ...
        <Button asChild variant="outline" className="mt-4 self-start">
          <Link href="/badges/new/email-exact">Restart</Link>
        </Button>
      </ResultShell>
    );
  }
  ...
  if (result.kind === "complete") {
    redirect("/profile?issued=email-exact");
  }
