...
describe("ministerCredentialType", () => {
  it("converts kebab slugs to PascalCase credential types", () => {
    expect(ministerCredentialType("email-domain")).toBe("MinisterEmailDomainCredential");
    expect(ministerCredentialType("oauth-account")).toBe("MinisterOauthAccountCredential");
    expect(ministerCredentialType("age-over-21")).toBe("MinisterAgeOver21Credential");
  });
...
describe("VC issue/verify round trip", () => {
  ...
  it("rejects a VC whose signature was made by a different issuer", async () => {
    ...
    const jwt = await issueVc(alpha, "email-domain", buildUserDid(alpha.domain, "u_x"), {
      domain: "example.com",
    });
  ...
  it("rejects a VC whose iss claim has been tampered (issuer mismatch)", async () => {
    ...
    const jwt = await issueVc(issuer, "email-domain", buildUserDid(issuer.domain, "u_iss"), {
      domain: "example.com",
    });
    ...
    await expect(verifyVc(wrongDomainIssuer, jwt)).rejects.toBeInstanceOf(VcVerificationError);
  });
  ...
  it("rejects a non-JWT input gracefully (no thrown noise)", async () => {
    ...
    await expect(verifyVc(issuer, "not.a.jwt")).rejects.toBeInstanceOf(VcVerificationError);
    await expect(verifyVc(issuer, "")).rejects.toBeInstanceOf(VcVerificationError);
  });
});
